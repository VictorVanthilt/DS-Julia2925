...
# ╔═╡ 77b9d40c-60b4-11f0-33cc-b76cdf362c9a
using PlutoUI, GLMakie, JLD2

# ╔═╡ eba6cced-39c1-4797-878f-6a756fbd9708
data = load("double_pendulum_data.jld2")["data"];

# ╔═╡ bd702a6e-3e02-4b2c-842a-e480c14b57af
md"""
# Double pendulum fractal
"""

# ╔═╡ 02a1a6e3-d7be-4a65-b6d4-e2f60de6755b
@bind norm_function Select(["2-norm", "1-norm theta 1", "1-norm theta 2"])

# ╔═╡ 232a4b0a-d47e-41bd-b441-27c018d011dd
t = @bind time PlutoUI.Slider(0:0.01:10; default=0, show_value=true)

# ╔═╡ 88599cc6-325d-47c3-baef-91db37feefe4
begin
	...
	tᵢ = findfirst(==(time), collect(0:0.01:10))
	data_0 = data[:,1,:]
	data_t = data[:,tᵢ,:]
	# distance = sqrt.((data_0[1, 1] .- (data_t[:, 1] .% π)).^2 + (data_0[1, 2] .- (data_t[:, 2] .% π)).^2)
	x = data_t[:, 1]
	y = data_t[:, 2]
	# heatmap!(ax, data_0[:, 1], data_0[:, 2], distance, colormap=:balance)
	if norm_function == "2-norm"
		distance = sqrt.((x .% π .- data[1,1,1]).^2 + (y .% π .- data[1,1,1]).^2)
	elseif norm_function == "1-norm theta 1"
		distance = abs.(x .- data_0[:, 1])
	elseif norm_function == "1-norm theta 2"
		distance = abs.(y .- data_0[:, 2])
	end
	heatmap!(ax, data_0[:, 1], data_0[:, 2], distance, colormap=:balance)
...
manifest_format = "2.0"
project_hash = "6744398c9b23c18b091fcb71942f7f7046803a37"
...
git-tree-sha1 = "1d7a18d5a4ac555962d0cf4ea34682ea83018a48"
